...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    // Business payments are confirmed by webhook — no polling needed, show success immediately
    if (businessId) { setConfirmed(true); return; }
    if (!listingId) { return; }
  ...
  return (
    <div style={{ maxWidth: 520, margin: "60px auto", padding: "0 20px", textAlign: "center" }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 700, color: "#26215C", margin: "0 0 12px" }}>
        {businessId ? "Your business is now featured!" : "Your listing is now featured!"}
      </h1>
      <p style={{ fontSize: 14, color: "#666", lineHeight: 1.7, margin: "0 0 32px" }}>
        {businessId
          ? "Your business will appear at the top of the directory for the next 7 days, reaching more people in the NepSaathi community."
          : "Your listing will appear at the top of search results for the next 7 days, reaching more people in the NepSaathi community."}
      </p>
      <div style={{
        background: "#E1F5EE",
        ...
        <CheckCircleIcon size={14} weight="fill" color="#085041" style={{ flexShrink: 0 }} />Payment confirmed · Featured for 7 days · Visible at the top of {businessId ? "the directory" : "search results"}
      </div>
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
        <Link
          to={businessId ? "/businesses" : "/my-listings"}
          style={{ background: "#534AB7", color: "#fff", padding: "12px 24px", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}
        >
          {businessId ? "View businesses" : "View my listings"}
        </Link>
        <Link
          to="/"
